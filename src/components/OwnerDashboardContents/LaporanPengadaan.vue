    ...
    <v-row row align="center" justify="center">
      <v-col cols="4" sm="4">
        <v-card color="#455A64" dark>
          <v-card-title class="headline">Laporan Pengadaan Produk Bulanan</v-card-title>

          <v-card-subtitle>Pilih Bulan dan Tahun pengadaan yang ingin dilihat.</v-card-subtitle>
      ...
      <v-col cols="4">
        <v-card color="#455A64" dark>
          <v-card-title class="headline">Laporan Pengadaan Produk Tahunan</v-card-title>

          <v-card-subtitle>Pilih Tahun pengadaan yang ingin dilihat.</v-card-subtitle>
      ...
      <v-card>
        <v-card-text>
          <v-container>
            ...
            <!-- <v-row justify="space-around">
              <v-text-field :value="formattedDate"></v-text-field>
              {{date}}
            </v-row>-->
          </v-container>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    printTahun() {
      ...
      var uri = this.$apiUrl + "laporan/pengadaanTahunan/" + this.tahun;
      this.$http
        .get(uri, { responseType: "blob" })
        .then(response => {
          const file = new Blob([response.data], { type: "application/pdf" });
